...
The current prototype’s “generated tile” (parent ghost + a child mosaic) is **Guide Generator output**, not default explorer chrome. That mosaic is N × N for a chosen child scale, not always 400 cells. Explorers should see clean published art. A later “workshop” toggle may show construction overlays; keep it out of v1.
...
| Scale up | Parent tile at a chosen parent scale, same world point. If more than one parent exists (e.g. Hood → Town or City), offer a chooser. |
| Scale down | Child tile at a chosen child scale. If more than one child scale exists (e.g. City → Town or Hood), offer a chooser; then the cell that contains the last click, or the centre child. |
